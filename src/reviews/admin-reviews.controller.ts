...
export class AdminReviewsController {
  ...
  async adminFindAllProduct(
    ...
    const { data, file_info } = await this.reviewsService.adminFindAllProduct(
      { take, page },
      search,
      order,
    );
  ...
  @ApiBody({
    description:
      'review idx를 배열로 전달 ex) [1,2,3]<br>' +
  ...
  async statusChange(@Body('idxs') idxs: [], @Body('status') status: string) {
  ...
  async starChange(@Body('idxs') idxs: [], @Body('star') star: string) {
  ...
  @HttpCode(204)
  async remove(@GetUser() user: UsersEntity, @Body('idxs') idxs: []) {
