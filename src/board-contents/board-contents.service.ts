...
export class BoardContentsService {
  ...
  async findAll(options: PaginationOptions) {
    ...
    const [results, total] = await this.bcRepository.findAndCount({
      ...
      where: { bc_status: bcConstants.regStatus, bc_type: In(bcConstants.notNoticeType) },
  ...
  async findNoticeAll() {
    return await this.bcRepository.find({
      order: { bc_createdAt: 'DESC' },
      where: { bc_status: bcConstants.regStatus, bc_type: bcConstants.noticeType },
      relations: ['user', 'board', 'bscats']
    });
  }
  ...
  async findCategoryAll(options: PaginationOptions, category: string) {
    ...
    const [results, total] = await this.bcRepository.findAndCount({
      ...
      where: (qb) => {
        ...
        qb.andWhere('bc_status = :bc_status', { bc_status: bcConstants.regStatus })
        qb.andWhere('bc_type IN (:bc_type)', { bc_type: bcConstants.notNoticeType })
      },
      relations: ['user', 'board', 'bscats'],
      take: take,
      skip: take * (page - 1)
  ...
  async findNoticeCategoryAll(category: string) {
    ...
    return await this.bcRepository.find({
      ...
      where: (qb) => {
        qb.where('bscat_bcat_idx = :bcat_idx', { bcat_idx: bcats[0].bcat_idx })
        qb.andWhere('bc_status = :bc_status', { bc_status: bcConstants.regStatus })
        qb.andWhere('bc_type = :bc_type', { bc_type: bcConstants.noticeType })
      },
      relations: ['user', 'board', 'bscats'],
    });
  }
  ...
  async findOne(idx: number) {
    ...
    if (bc['bc_status'] !== bcConstants.regStatus) {
  ...
  getPrivateModel(): any[] {
    const userPrivateModel = this.usersService.getPrivateModel();
    return [
      ...bcConstants.privateModel,
      ...userPrivateModel
    ];
  }
}
