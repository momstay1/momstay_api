...
export class OrderController {
  ...
  @ApiOperation({
    summary: '주문 생성 API',
    description:
      '최초 실행시 productOptionIdx값만 입력<br>' +
  ...
  @ApiOperation({
    ...
    description:
      '관리자 계정 로그인 상태에서 리스트 조회시 다른 사람의 주문 내역 확인 가능<br>' +
  ...
  async guestFindAll(
    ...
    return await this.orderService.guestFindAll(
      user,
      { take, page },
      search,
      order,
  ...
  async hostFindAll(
    ...
    return await this.orderService.hostFindAll(
      user,
      { take, page },
      search,
      order,
  ...
  @Get('host/excel')
  @ApiOperation({
    summary: '호스트 주문 리스트 엑셀 다운로드 API',
  })
  @Auth(['root', 'admin', 'host'])
  @ApiBearerAuth()
  @ApiQuery({
  ...
  async hostExcelDownload(
    ...
    const data = await this.orderService.hostExcelDownload(
      user,
      { take, page },
      search,
      order,
  ...
  async guestExcelDownload(
    ...
    const data = await this.orderService.guestExcelDownload(
      user,
      { take, page },
      search,
      order,
  ...
  @ApiOperation({
    ...
    description:
      '게스트가 바로결제한 주문의 상세 조회 모든 계정 조회 가능<br>' +
  ...
  @ApiOperation({
    ...
    description:
      '호스트가 관리하는 숙소의 상세 조회 자신의 숙소 주문 상세만 조회 가능<br>' +
      '게스트 계정은 권한 없음<br>' +
  ...
  async findOneCodeByNonmember(@Param('code') code: string) {
