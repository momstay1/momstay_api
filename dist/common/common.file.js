...
const createFolder = (folder) => {
    ...
    try {
        fs.mkdirSync(common_constants_1.commonContants.upload_path);
    }
    catch (error) {
    ...
    try {
        ...
        fs.mkdirSync(path.join(common_constants_1.commonContants.upload_path, `${folder}`));
    }
    catch (error) {
...
const storage = () => {
    return multer.diskStorage({
        destination(req, file, cb) {
            ...
            createFolder(folder);
            const folder_name = path.join(common_constants_1.commonContants.upload_path, folder);
            cb(null, folder_name);
        },
