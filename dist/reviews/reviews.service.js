...
let ReviewsService = class ReviewsService {
    constructor(reviewRepository, productService, userService, fileService, excelService) {
    ...
    async create(userInfo, createReviewDto, files) {
        if ((0, lodash_1.get)(createReviewDto, 'depth', 0) > 0 &&
            (0, lodash_1.get)(createReviewDto, 'group', 0) == 0) {
            throw new exceptions_1.UnprocessableEntityException('reviews.service.create: 처리 할 수 없습니다.');
        }
    ...
    async findAllIdxs(idxs) {
        if (idxs.length <= 0) {
            throw new exceptions_1.NotFoundException('reviews.service.findAllIdxs: 잘못된 정보 입니다.');
        }
        ...
        if (reviews.length <= 0) {
            throw new exceptions_1.NotFoundException('reviews.service.findAllIdxs: 정보를 찾을 수 없습니다.');
        }
        return reviews;
    }
    ...
    async adminFindAllProduct(options, search, order) {
        const { take, page } = options;
        const where = common_utils_1.commonUtils.searchSplit(search);
        ...
        const [results, total] = await this.reviewRepository
            ...
            qb.andWhere('`review`.`depth` = :depth', { depth: depthZero });
            (0, lodash_1.get)(where, 'star', '') && qb.andWhere('`review`.`star` IN (:star)', { star: (0, lodash_1.isArray)(where['star']) ? where['star'] : [where['star']] });
            (0, lodash_1.get)(where, 'name', '') && qb.andWhere('`user`.`name` LIKE :name', { name: '%' + where['name'] + '%' });
            (0, lodash_1.get)(where, 'id', '') && qb.andWhere('`user`.`id` LIKE :id', { id: '%' + where['id'] + '%' });
            (0, lodash_1.get)(where, 'title', '') && qb.andWhere('`product`.`title` LIKE :title', { title: '%' + where['title'] + '%' });
            (0, lodash_1.get)(where, 'min_createdAt', '') && qb.andWhere('`review`.`createdAt` >= :min_createdAt', { min_createdAt: where['min_createdAt'] });
            (0, lodash_1.get)(where, 'max_createdAt', '') && qb.andWhere('`review`.`createdAt` <= :max_createdAt', { max_createdAt: where['max_createdAt'] });
        })
            .orderBy(order_by)
            .skip(take * (page - 1) || 0)
            .take(take || 10)
    ...
    async averageStar(review) {
        ...
        const average_star = star_data[0].review_cnt != 0
    ...
    async findOneIdx(idx) {
        if (!idx) {
            throw new exceptions_1.NotFoundException('reviews.service.findOneIdx: 잘못된 정보 입니다.');
        }
        ...
        if (!(0, lodash_1.get)(review, 'idx', '')) {
            throw new exceptions_1.NotFoundException('reviews.service.findOneIdx: 정보를 찾을 수 없습니다.');
        }
        return review;
    }
    async findOne(idx) {
        ...
        if (review['status'] == deleteStatus) {
            throw new exceptions_1.NotFoundException('reviews.service.findOne: 삭제된 후기 입니다.');
        }
        let file_info = {};
        try {
    ...
    async update(idx, userInfo, updateReviewDto, files) {
        ...
        if (!common_utils_1.commonUtils.isAdmin(user['group']['id'])) {
            if (prevReview['user']['idx'] != user['idx']) {
                throw new exceptions_1.UnauthorizedException('reviews.service.update: 권한이 없습니다.');
            }
        }
    ...
    async starChange(idxs, star) {
        if (idxs.length <= 0 || !star) {
            throw new exceptions_1.NotFoundException('review.service.starChange: 변경할 정보가 없습니다.');
        }
    ...
    async statusChange(idxs, status) {
        if (idxs.length <= 0 || !status) {
            throw new exceptions_1.NotFoundException('review.service.statusChange: 변경할 정보가 없습니다.');
        }
        const reivews = await this.findAllIdxs(idxs);
        if (reivews.length <= 0) {
            throw new exceptions_1.NotFoundException('review.service.statusChange: 변경할 정보가 없습니다.');
    ...
    async remove(idxs, userInfo) {
        if (idxs.length <= 0) {
            throw new exceptions_1.NotFoundException('review.service.statusUpdate: 삭제할 정보가 없습니다.');
        }
        const user = await this.userService.findId((0, lodash_1.get)(userInfo, 'id'));
        const reivews = await this.findAllIdxs(idxs);
        if (!common_utils_1.commonUtils.isAdmin(user['group']['id'])) {
            if (idxs.length > 1 || reivews.length > 1) {
                throw new exceptions_1.UnprocessableEntityException('review.service.statusUpdate: 삭제할 수 없습니다.');
            }
            if (reivews[0]['user']['idx'] != user['idx']) {
                throw new exceptions_1.UnauthorizedException('review.service.statusUpdate: 권한이 없습니다.');
            }
        }
    ...
    async createExcel(options, search, order) {
        ...
        if (!reviews) {
            throw new exceptions_1.NotFoundException('review.service.excel: 다운로드할 데이터가 없습니다.');
        }
